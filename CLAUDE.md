...
- **⚠️ `migrate.js` неидемпотентен (roadmap 4.11).** В JSON 117 рецептов — совпадает с БД, так что потерь сейчас нет. Но `INSERT INTO recipes` без `OR IGNORE` и без `UNIQUE(name)` — повторный запуск задублирует. `COMMIT` стоит в верхнеуровневом `serialize`, а рецепты/связи вставляются через вложенные колбэки `db.get(...)` — формально гонка, в текущих условиях скрывается порядком очереди sqlite3, но на fast-path ломается. Переписать на `async/await`, добавить assert количества в конце.
...
- **✅ Хаос в `unit` — нормализован словарём (roadmap 4.13).** `database/unit-aliases.json` + `normalizeUnit` в `migrate.js` сворачивают `ст.л.`/`ст. л.`, `стакан`/`стаканов`/`чашка`, срезают суффикс `(150 г)` и т.п. После нормализации — 20 вариантов вместо 37. Применится окончательно после Stage 0.5 (полный реимпорт).
- **✅ Задвоение ингредиентов в справочнике — нормализовано словарём (roadmap 4.1).** `database/ingredient-aliases.json` + `normalizeIngredient` в `migrate.js`: `Лук`/`Лук репчатый` → `Лук репчатый`, `Красный лук`/`Лук сладкий`/`Лук красный` → `Лук красный` и т.д. (13 канонических групп). 185 ингредиентов сжимаются до 170. Применится окончательно после Stage 0.5.
